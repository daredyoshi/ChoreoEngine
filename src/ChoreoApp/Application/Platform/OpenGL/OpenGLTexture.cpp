...
ChoreoApp::OpenGLTexture2D::OpenGLTexture2D(uint32_t width, uint32_t height)
...
void ChoreoApp::OpenGLTexture2D::setdata(void* data, uint32_t size) {
    CE_PROFILE_FUNCTION();  
...
ChoreoApp::OpenGLTexture2D::OpenGLTexture2D(const std::string& path)
    : m_path(path) { 
    CE_PROFILE_FUNCTION();  
...
ChoreoApp::OpenGLTexture2D::~OpenGLTexture2D(){
    CE_PROFILE_FUNCTION();  
    glDeleteTextures(1, &m_rendererId);
}

void ChoreoApp::OpenGLTexture2D::bind(uint32_t slot) const  
{
    CE_PROFILE_FUNCTION();  
