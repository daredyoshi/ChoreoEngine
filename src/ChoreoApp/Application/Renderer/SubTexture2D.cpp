...
ChoreoApp::SubTexture2D::SubTexture2D(const Ref<Texture2D>& texture, const glm::vec2& min, const glm::vec2& max)
    :
        m_texture{texture}        
{
    m_uvs[0] = {min.x, min.y}; 
...
ChoreoApp::Ref<ChoreoApp::SubTexture2D> ChoreoApp::SubTexture2D::createFromCoords(const Ref<Texture2D>& texture , const glm::vec2 spriteUnitCoords, const glm::vec2 spriteUnitDimensions, const unsigned int spriteUnitPixels){
